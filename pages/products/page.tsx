...
export default function Products() {
  ...
  const { data: total } = useQuery<{ items: number }, unknown, number>(
    ...
    () =>
      fetch(`/api/get-products-count?category=${selectedCategory}&contains=${debouncedKeyword}`).then((res) =>
        res.json()
      ),
    {
  ...
  return (
    <div className="px-36 mt-36 mb-36">
      <div className="mb-4">
        <Input icon={<IconSearch />} placeholder="Search" value={keyword} onChange={handleChange} />
      </div>
      <div className="mb-4">
      ...
      <div className="w-full flex mt-5">
        {total && <Pagination className="m-auto" value={activePage} onChange={setPage} total={total} />}
      </div>
    </div>
  );
}
