  ...
  const handleChange = () => {
    if (inputRef.current && inputRef.current.files && inputRef.current.files.length > 0) {
      for (let i = 0; i < inputRef.current.files.length; i++) {
        ...
        fetch('https://api.imgbb.com/1/upload?key=70bc571491a05e670ff190867d29f5eb&expiration=15552000', {
          method: 'POST',
          body: fd,
        })
          .then((res) => res.json())
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex' }}>
        {images && images.length > 0 && images.map((image, idx) => <AutoSizeImage key={idx} src={image} />)}
      </div>
    </div>
  );
}
