  ...
  return (
    <div>
      ...
      <div className="flex">
        ...
        <div className="px-4">
          <div className="flex flex-col p-4 space-y-4" style={{ minWidth: 300, border: '1px solid grey' }}>
            <div>Info</div>
            <Row>
              <span>금액</span>
            ...
            <Row>
              ...
              <span className="font-semibold text-red-500">
                {(amount + dilveryAmount - discountAmount).toLocaleString('ko-kr')} 원
              </span>
            </Row>
  ...
  const { mutate: updateCart } = useMutation<unknown, unknown, Cart, any>(
        ...
        .then((res) => res.items),
    {
      onMutate: async (item) => {
        ...
        queryClient.setQueryData<Cart[]>([CART_QUERY_KEY], (old) => old?.filter((c) => c.id !== item.id).concat(item));
